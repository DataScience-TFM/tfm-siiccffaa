...
SOURCE = "`tfm-sbs.siiccffaa`"
CLEAN = "`tfm-sbs.siiccffaa_clean`"
...
STATEMENTS = [
    ...
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_region AS
    SELECT
      id AS region_id,
    ...
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_provincia AS
    SELECT
      p.id AS provincia_id,
      REGEXP_REPLACE(TRIM(p.name), r'\\s+', ' ') AS provincia_name,
      p.region AS region_id,
      r.region_name
    FROM {SOURCE}.provincias p
    LEFT JOIN {CLEAN}.dim_region r ON r.region_id = p.region
    WHERE p.id IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY p.id ORDER BY p.updated_at DESC, p.created_at DESC) = 1
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_municipio AS
    SELECT
      ...
      p.region_id,
      p.region_name
    FROM {SOURCE}.municipios m
    LEFT JOIN {CLEAN}.dim_provincia p ON p.provincia_id = m.provincia
    WHERE m.id IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY m.id ORDER BY m.updated_at DESC, m.created_at DESC) = 1
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_report_type AS
    SELECT
      ...
      require_validation,
      ext_table_name
    FROM {SOURCE}.report_types
    WHERE id IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY id ORDER BY updated_at DESC, created_at DESC) = 1
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_company AS
    SELECT
      id AS company_id,
    ...
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_branch AS
    SELECT
      b.id AS branch_id,
    ...
    FROM {SOURCE}.branches b
    LEFT JOIN {CLEAN}.dim_company c ON c.company_id = b.company
    WHERE b.id IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY b.id ORDER BY b.updated_at DESC, b.created_at DESC) = 1
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_company_department AS
    SELECT
    ...
    FROM {SOURCE}.company_departments d
    LEFT JOIN {CLEAN}.dim_company c ON c.company_id = d.company
    WHERE d.id IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY d.id ORDER BY d.updated_at DESC, d.created_at DESC) = 1
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.dim_report_group AS
    SELECT
    ...
    FROM {SOURCE}.report_groups g
    LEFT JOIN {CLEAN}.dim_company c ON c.company_id = g.company
    WHERE g.id IS NOT NULL
    QUALIFY ROW_NUMBER() OVER (PARTITION BY g.id ORDER BY g.updated_at DESC, g.created_at DESC) = 1
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.fact_reportes_limpios
    ...
    WITH provincia_por_reporte AS (
      ...
      FROM {SOURCE}.reports__provincias
      GROUP BY report_id
    ),
    reportes_base AS (
      SELECT
    ...
    FROM texto t
    LEFT JOIN {CLEAN}.dim_report_type rt ON rt.report_type_id = t.report_type
    LEFT JOIN {CLEAN}.dim_company c ON c.company_id = t.company
    LEFT JOIN {CLEAN}.dim_branch b ON b.branch_id = t.branch
    LEFT JOIN {CLEAN}.dim_company_department cd ON cd.company_department_id = t.company_department
    LEFT JOIN {CLEAN}.dim_report_group rg ON rg.report_group_id = t.report_group
    LEFT JOIN {CLEAN}.dim_provincia p ON p.provincia_id = COALESCE(t.provincia, t.provincia_id_puente)
    LEFT JOIN {CLEAN}.dim_region rgion ON rgion.region_id = COALESCE(t.region, p.region_id)
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.agg_reportes_semanal
    ...
    WITH base AS (
      ...
      FROM {CLEAN}.agg_reportes_semanal
    ...
    """,
    f"""
    CREATE OR REPLACE TABLE {CLEAN}.data_quality_summary AS
...
def run(statement: str, idx: int) -> None:
    sql = " ".join(line.strip() for line in statement.strip().splitlines() if line.strip())
    print(f"\n[{idx:02d}/{len(STATEMENTS):02d}] Ejecutando BigQuery...")
    cmd = [
        BQ,
